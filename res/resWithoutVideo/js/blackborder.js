...
$('#redmarker').click(redmarker);
$('#blackmarker').click(blackmarker);
$('#whitemarker').click(whitemarker);
$('#clear').click(clearBorder);
...
function draw_line(x1, y1, x2, y2) 
{
	...
	ctx.lineTo(x2, y2);
	ctx.stroke();
	//Запись о рисовании (нужна ли проверка? вопрос риторический)
...
function init(pic) 
{
	...
	dctrl = { drawing: false };
	//События мышки
	$('#blackboard').on('mousedown', canvas_mousedown);
	$('#blackboard').on('mousemove', canvas_mousemove);
	$('#blackboard').on('mouseup', canvas_mouseup);
	//Размер картинки
	$y= canvas.height;
	$x= canvas.width;
