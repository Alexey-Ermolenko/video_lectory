...
{
	echo '<tr><td><a class="links" href="index.php?id='.$row['id'].'">'.$row['name'].'</a></td><td>' . $row['specification'] .'</td><td>' . $row['autor'] .'</td></tr>';
}
//echo '<tr><td><a class="links" href="newLection.html">Создать новую лекцию</a></td><td></td><td></td></tr>';
echo '</table>';
//Закрытие соединения
mysqli_close($link);
?>